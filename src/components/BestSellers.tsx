...
import { Star, Clock, ShoppingCart, Percent, Edit2, Trash2, AlertTriangle, Sparkles } from 'lucide-react';
  ...
  return (
    <section id="our-bread" className="py-20 bg-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Products Grid */}
        {products.length === 0 ? (
          <div className="text-center py-16 px-6 bg-white rounded-[2rem] border border-dashed border-chocolate/20 max-w-xl mx-auto space-y-4 shadow-sm">
            <div className="w-16 h-16 bg-beige/50 border border-chocolate/5 rounded-2xl flex items-center justify-center mx-auto text-chocolate/60">
              <Sparkles className="w-8 h-8 text-caramel" />
            </div>
            <h3 className="font-serif text-xl font-bold text-chocolate">Menu currently being prepared</h3>
            <p className="text-xs sm:text-sm text-chocolate/70 leading-relaxed font-sans max-w-sm mx-auto">
              We are currently selecting and updating today's fresh baked goods menu. Check back shortly, or log in as an administrator to publish new delicious recipes to the active menu!
            </p>
          </div>
        ) : (
